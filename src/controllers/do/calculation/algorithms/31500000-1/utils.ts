...
import type { AvailableVariant, AvailableVariants } from 'types/transactions/available-variants';
import type { Calculation } from './types';

const categoryId = '31500000-1';
export const { techChars, calculateEnergyEfficiencyClass } = refData[categoryId];

export const getValueFromResponse = (responses: RequirementResponse[], requirementId: string): unknown => {
...
export const getPowerRef = (lum: number): number => {
  return lum >= 1300 ? 0.07341 * lum : 0.88 * Math.sqrt(lum + 0.49 * lum);
};
...
export const generateAvailableVariants = (
  ...
  const notSortedVariants: AvailableVariants['availableVariants'] = (Object.keys(availableBulbTypes) as Variants[]).map(
    (bulbType) => {
  ...
  return [
    notSortedVariants.find((variant) => variant.relatedItem === selectedBulbType) as AvailableVariant,
    ...notSortedVariants
